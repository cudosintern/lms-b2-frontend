...
import { SetCourseEnrollLimitModal } from "./SetCourseEnrollLimitModal";
import type { ApiResponse, Course, CourseStructure, Curriculum, Department, EnrollResponse, Program, RegistrationSetupResponse, Term } from "./studentCourseRegistration.types";
...
interface LegacyCourseStructure {
  course_type: string;
...
interface LegacyEnrollResponse {
  semester_id: number;
  course_type: string;
...
interface LegacyRegistrationSetupResponse {
  status: boolean;
  data: {
    semester: {
      start_date: string;
...
const LegacySetCourseEnrollLimitModal: React.FC<SetCourseEnrollLimitModalProps> = ({
  open,
  curriculumName,
  termName,
  courseType,
    ...
    <div style={modalStyles.overlay} onClick={onClose}>
      <div style={modalStyles.modal} onClick={(e) => e.stopPropagation()}>
        <div style={modalStyles.header}>
          <h2 style={modalStyles.title}>Course Enrollment Details</h2>
        </div>
...
const DepartmentDropdown = () => {
  ...
  const parseServerDate = (value?: string): Dayjs | null => {
    if (!value) return null;
    const parsed = dayjs(value, ["DD-MM-YYYY", "YYYY-MM-DD"], true);
    return parsed.isValid() ? parsed : null;
  };

  const parseServerTime = (value?: string): Dayjs | null => {
    if (!value) return null;
    const parsed = dayjs(value, ["hh:mm A", "HH:mm", "HH:mm:ss"], true);
    return parsed.isValid() ? parsed : null;
  };
  ...
  const loadRegistrationData = async (semesterId: string) => {
    ...
    try {
      ...
      if (response.data.status && response.data.data) {
        ...
        // Set dates and times
        setStartDate(parseServerDate(semester.start_date));
        setStartTime(parseServerTime(semester.start_time));
        setEndDate(parseServerDate(semester.end_date));
        setEndTime(parseServerTime(semester.end_time));
        ...
        if (course_structure && course_structure.length > 0) {
          ...
          course_structure.forEach((item) => {
            if (item.course_type) {
              ...
              studentsObj[item.course_type] = String(item.max_students ?? 0);
            }
          });
  ...
  const validateAllFields = (): string | null => {
    const total = Number(editableTotalCredits);
    const minCredits = Number(editableMinCredits);
    ...
    if (startDate && endDate) {
      const startsAt = dayjs(`${startDate.format("YYYY-MM-DD")} ${startTime?.format("HH:mm") || "00:00"}`);
      const endsAt = dayjs(`${endDate.format("YYYY-MM-DD")} ${endTime?.format("HH:mm") || "23:59"}`);
    ...
    if ((startDate || startTime || endDate || endTime) &&
      ...
      return '❌ Provide a start date and time and an end date and time';
    }

    for (const [label, value] of [
      ['Own-curriculum electives', editableOwnElectives],
      ['Other-curriculum electives', editableOtherElectives],
    ]) {
      const electiveCount = Number(value);
    ...
    for (const item of courseStructure) {
      const min = Number(editableCourseMinCredits[item.course_type]);
      const max = Number(editableCourseMaxCredits[item.course_type]);
      const courseTotal = item.total_credits || 0;
      const courseType = item.course_type;

      if (!Number.isFinite(min) || !Number.isFinite(max)) {
        errorMessages.push(`❌ Min and Max credits are required for "${courseType}"`);
        continue;
      }
      ...
      const maxStudents = Number(editableMaxStudents[courseType]);
      if (!Number.isInteger(maxStudents) || maxStudents < 0) {
        errorMessages.push(`❌ Max students must be a whole number of 0 or more for "${courseType}"`);
      }
  ...
  return (
    <>
          ...
          <div className="form-row-datetime">
            <div className="form-group">
              ...
              <input
                ...
                onChange={(event) => setStartDate(event.target.value ? dayjs(event.target.value) : null)}
                disabled={loading}
              />
            </div>
            ...
            <div className="form-group">
              ...
              <input
                ...
                onChange={(event) => setStartTime(event.target.value ? dayjs(`1970-01-01T${event.target.value}`) : null)}
                disabled={loading}
              />
            </div>
            ...
            <div className="form-group">
              ...
              <input
                ...
                onChange={(event) => setEndDate(event.target.value ? dayjs(event.target.value) : null)}
                disabled={loading}
              />
            </div>
            ...
            <div className="form-group">
              ...
              <input
                ...
                onChange={(event) => setEndTime(event.target.value ? dayjs(`1970-01-01T${event.target.value}`) : null)}
                disabled={loading}
              />
            </div>
                    ...
                    <tr key={index}>
                      ...
                      <td>
                        <input
                          ...
                          value={editableMaxStudents[item.course_type] ?? ""}
                          onChange={(e) => setEditableMaxStudents({
                            ...editableMaxStudents,
                            [item.course_type]: e.target.value,
                          })}
                          min="0"
                          step="1"
                          disabled={loading}
      ...
      <p className="note">
        2. The sum of Min and Max values must not exceed the total number of credits students can enroll.
      </p>
    </div>
    ...
    <div className="button-section">
      <div className="button-row">
        ...
        <button className="cancel-btn" onClick={() => selectedTerm && loadRegistrationData(selectedTerm)} disabled={loading}>
          Cancel
        </button>
      </div>
