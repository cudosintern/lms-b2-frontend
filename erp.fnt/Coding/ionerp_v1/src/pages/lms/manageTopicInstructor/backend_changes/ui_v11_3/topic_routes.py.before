...
def cudos_topics(request: TopicContext, db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
    ...
    return success([{'topic_id': t.topic_id, 'topic_code': t.topic_code, 'topic_title': t.topic_title,
        'topic_hrs': t.topic_hrs, 'num_of_sessions': t.num_of_sessions,
        'instructor_ids': [m.instructor_id for m in mapping_query(db, request).filter_by(topic_id=t.topic_id).all()]}
        for t in topic_query(db, request).all()
        if (bool(t.category_id) == bool(section.parent_id))])
